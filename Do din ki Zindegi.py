...
def start_game():
    ...
    # 2 sphere-type enemies (original type)
    for i in range(2):
        enemies.append({
...
def player_drawing():
    ...
    glPushMatrix()
    glScalef(20, 10, 30)                                    
    glutSolidCube(1)         # Take cube size as the parameter and make a cube. 
    glPopMatrix()
...
def keyboardListener(key, x, y):
    ...
    if not game_paused and not game_over:
        ...
        plyr_pos[0] = max(-550, min(plyr_pos[0], 550))     # ensuring player doesnt go beyond the walls
...
def setupCamera():
    glMatrixMode(GL_PROJECTION)
    glLoadIdentity()
    gluPerspective(fovY, 1.25, 0.1, 1500)          # (field of view, aspect ratio, Znear, Zfar)
    glMatrixMode(GL_MODELVIEW)
    glLoadIdentity()
...
def main():
    ...
    glutCreateWindow(b"Do din ki Zindegi")
